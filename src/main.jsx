...
function Header({page,setPage}){
  ...
  return <header className="marketingHeader">
    ...
    <nav className={open?'open':''}>
      {links.map(([label,p])=><button key={p} className={page===p?'active':''} onClick={()=>{setPage(p);setOpen(false)}}>{label}</button>)}
    </nav>
...
function Analysis({setPage,resultInput,onResult}){
  ...
  const items=['Checking your business information','Reviewing uploaded documents','Calculating cash-flow pressure','Stress-testing repayment capacity','Comparing restructuring scenarios'];

  useEffect(()=>{
    let alive=true;
    const timer=setInterval(()=>setActive(v=>Math.min(v+1,items.length)),650);
    let redirectTimer;
    (async()=>{
      try{
        ...
        const minimumView=3200;
        redirectTimer=setTimeout(()=>setPage('dashboard'),Math.max(0,minimumView-elapsed));
      }catch(err){
        if(!alive)return;
    ...
    return()=>{alive=false;clearInterval(timer);if(redirectTimer)clearTimeout(redirectTimer)};
  },[resultInput,onResult,setPage,startedAt]);

  const done=status==='success';
  return <main className="page analysisPage"><div className="analysisCard analysisProcessCard">
    <div className="analysisOrb"><div className="orbInner"><Sparkles size={27}/></div></div>
    <Pill tone={done?'green':status==='error'?'red':''}>{done?'ASSESSMENT READY':status==='error'?'ANALYSIS NEEDS ATTENTION':'ANALYSING YOUR BUSINESS'}</Pill>
    <h1>{done?'Your restructuring picture is ready.':status==='error'?'We couldn’t complete the analysis.':'Building your restructuring picture…'}</h1>
    <p>{done?`${resultInput?.form?.companyName || 'Your business'} has been analysed. Taking you to the overview now.`:status==='error'?error:'We’re turning your simple answers and documents into a clear cash-flow and repayment picture.'}</p>
    <div className="checks compactChecks">{items.map((x,i)=><div className={i<active?'checked':''} key={x}><span>{i<active?<Check size={14}/>:<Clock3 size={14}/>}</span>{x}{i<active&&<small>Done</small>}</div>)}</div>
    {done?<div className="analysisReady"><Check size={16}/><span>Analysis complete. Opening your restructuring picture…</span></div>:status==='error'?<div className="analysisError"><AlertTriangle size={17}/><span>{error}</span><button className="ghost" onClick={()=>window.location.reload()}>Try again</button></div>:<div className="analysisWait">Analysing securely in the background…</div>}
  </div></main>
}
...
function App(){
  ...
  const startNew=()=>{
    setResultInput(null);
    setAssessment({data:demoData,mode:'demo'});
    setPage('intake');
  };
  ...
  const renderPage=()=>{
    ...
    if(page==='intake') return <Intake setPage={setPage} setResultInput={setResultInput}/>;
    if(page==='analysis') return <Analysis setPage={setPage} resultInput={resultInput} onResult={setAssessment}/>;
    if(page==='dashboard') return <Dashboard setPage={setPage} data={assessment.data}/>;
    if(page==='scenarios') return <Scenarios data={assessment.data}/>;
    if(page==='bank') return <Bank data={assessment.data}/>;
    return null;
  };

  const showFooter=page==='landing'||page==='intake';

  return <>
    <Header page={page} setPage={setPage}/>
    <div className="modeBadge">{liveMode?'LIVE N8N DATA':'PROTOTYPE · BACKEND OFF'}</div>
    {renderPage()}
    {showFooter&&<footer>
      <button className="brandButton footerBrand" onClick={startNew}><Logo/></button>
      <span>ASK SIMPLE. ANALYSE DEEPLY. EXPLAIN SIMPLY.</span>
      <span>Illustrative decision support · © 2026</span>
    </footer>}
  </>;
}
